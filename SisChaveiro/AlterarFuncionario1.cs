...
namespace SisChaveiro
{
    public partial class AlterarFuncionario1 : Form
    {
        ...
        private void SelecioneFuncionario1(object sender, DataGridViewCellEventArgs e)
        {
            ...
            Ide = Convert.ToInt32(data1.Rows[e.RowIndex].Cells[0].Value);
            nomef = Convert.ToString(data1.Rows[e.RowIndex].Cells[1].Value);
            Cpf = Convert.ToString(data1.Rows[e.RowIndex].Cells[2].Value);
            rua = Convert.ToString(data1.Rows[e.RowIndex].Cells[3].Value);
            Num = Convert.ToInt32(data1.Rows[e.RowIndex].Cells[4].Value);
            Cep = Convert.ToString(data1.Rows[e.RowIndex].Cells[5].Value);
            complemento = Convert.ToString(data1.Rows[e.RowIndex].Cells[6].Value);
            bairro = Convert.ToString(data1.Rows[e.RowIndex].Cells[7].Value);
            cidade = Convert.ToString(data1.Rows[e.RowIndex].Cells[8].Value);
            estado = Convert.ToString(data1.Rows[e.RowIndex].Cells[9].Value);
