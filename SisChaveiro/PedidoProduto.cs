...
namespace SisChaveiro
{
    public partial class PedidoProduto : Form
    {
        ...
        {
            string selecionar = @"SELECT Id_Produto, Modelo, Preco_Unitario FROM
        ...
        private void Selecione(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (selecao == 2)
            {
                ...
                modelo = Convert.ToString(data.Rows[e.RowIndex].Cells[1].Value);
                txtModelo.Text = modelo;

                precou = Convert.ToDouble(data.Rows[e.RowIndex].Cells[2].Value);
